...
| 과제                        | 컴포넌트                     | 구현 내용                                                                                                                                                               |
| --------------------------- | ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Hands on 1 — Weather Mockup | `exercise/WeatherMockup.vue` | `v-for` 날씨 카드 목록(`:key` 고유 id) / `v-if` 기온 25도 기준 라벨 분기 / `:value`+`@input` 한글 즉시 동기화 검색 / 카드 클릭 상태바 갱신 및 `@click.stop` 버블링 차단 |
