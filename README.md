...
종합과제와 단원별 실습을 별도 페이지로 분리하고, 상단 내비게이션 바로 전환합니다.

| 경로         | 화면                                    | 로딩 방식                                                       |
| ------------ | --------------------------------------- | --------------------------------------------------------------- |
| `/`          | 🌤️ 종합과제 (Weather Mockup)            | 정적 import — 첫 진입 페이지                                    |
| `/practices` | 📚 단원별 실습 (1~6단원, 27개 컴포넌트) | **동적 import (Lazy Loading)** — 해당 경로 진입 시에만 내려받음 |

실습 화면은 컴포넌트 수가 많아 지연 로딩을 적용했으며, 빌드 시 `PracticeView` 청크가
별도 파일로 분리되어 초기 번들 크기를 낮춥니다.
